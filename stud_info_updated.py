...
while True:
    ...
    if ch == 1:
        ...
        def fetch_table_data(self,stud_info):
             PRODUCT_FETCH_QUERY = '''SELECT * stud_info'''
             channel1 = connection.cursor()
             channel1.execute(PRODUCT_FETCH_QUERY)
             print(channel1.fetchall())
        ...
        def export(stud_info):
            workbook1 = xlsxwriter.Workbook(stud_info + 'student.xlsx')
            worksheet = workbook1.add_worksheet('MENU')

            # Create style for cells
            header_cell_format = workbook1.add_format({'bold': True, 'border': True, 'bg_color': 'yellow'})
            body_cell_format = workbook1.add_format({'border': True})
            ...
            print(str(row_index) + ' rows written successfully to ' + workbook1.filename)
